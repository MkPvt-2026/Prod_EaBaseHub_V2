...
import { CURRENT_SUPABASE } from "/js/services/appConfig.js";

export async function sendLineNotify(data) {
  try {
    console.log("📨 LINE ENV =", CURRENT_SUPABASE.env);

    const res = await fetch(
      `${CURRENT_SUPABASE.url}/functions/v1/send-line-notify`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${CURRENT_SUPABASE.anonKey}`,
        },
