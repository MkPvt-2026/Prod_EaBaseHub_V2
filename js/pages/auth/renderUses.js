...
function buildRow(tmpl, user, index) {
    ...
    // ── เติมข้อมูลข้อความ ─────────────────────────────────────
    setField(clone, "email", user.email || "-");
    setField(clone, "username", user.username || "-");
    setField(clone, "display_name", user.display_name || "-");
    // ── Role Badge ────────────────────────────────────────────
    ...
    // ── Toggle Icon + Label ───────────────────────────────────
    setField(clone, "toggle-icon", isActive ? "block" : "check_circle");
    setField(clone, "toggle-label", isActive ? "ระงับ" : "เปิดใช้");
    // ── Toggle Button class ───────────────────────────────────
    const toggleBtn = clone.querySelector("[data-action='toggle']");
    if (toggleBtn) {
        toggleBtn.classList.add(isActive ? "btn-toggle-inactive" : "btn-toggle-active");
        // ผูก event toggle status
        toggleBtn.addEventListener("click", () => toggleStatus(user.id, isActive ? "Inactive" : "Active"));
    }
    // ── ผูก Event ─────────────────────────────────────────────
    // ปุ่ม edit
    ...
    if (delBtn)
        delBtn.addEventListener("click", () => openDeleteModal(user.id, user.email || user.username || ""));
    return clone;
}
// ============================================================
// setField()
...
//# sourceMappingURL=renderUses.js.map