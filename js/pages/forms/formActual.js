// =====================================================
// formActual.js v2.4 — ใบเดินทางจริงและเคลียร์ค่าใช้จ่าย ๒
...
async function loadUserInfo() {
  try {
    ...
    const el = document.getElementById("sidebarEmail"); if (el) el.textContent = session.user.email;
    const { data: profile } = await supabaseClient.from("profiles").select("display_name, area").eq("id", session.user.id).single();
    const empEl = document.getElementById("empName"), zoneEl = document.getElementById("empZone");
...
async function checkLatestPlan() {
  try {
    ...
    const { data, error } = await supabaseClient.from("trips").select("id, user_name, start_date, end_date, area, trips, status, created_at").eq("user_id", session.user.id).order("created_at", { ascending: false }).limit(1);
    if (error || !data || data.length === 0) return;
    planData = data[0]; currentPlanId = planData.id;
    const banner = document.getElementById("planBanner"), banTx = document.getElementById("planBannerText"), refInp = document.getElementById("refPlanId");
    if (refInp) refInp.value = planData.id; if (banner) banner.style.display = "flex";
    ...
    if (banTx) {
      ...
      banTx.textContent = `พบแผนการเดินทาง (ฟอร์ม ๑) : ${fmtD(planData.start_date)}` + (planData.end_date ? ` – ${fmtD(planData.end_date)}` : "") + ` (${tripCount} แถว)`;
    }
...
function importFromPlan() {
  ...
  if (Array.isArray(planData.trips)) { tripRows = planData.trips; }
  else if (planData.trips && typeof planData.trips === "object") { tripRows = planData.trips.rows || []; expense = planData.trips.expense || null; }
  else { tripRows = []; }
  if (tripRows.length === 0) { alert("แผนนี้ยังไม่มีข้อมูลแถวเดินทาง"); return; }
  document.getElementById("tableBody").innerHTML = "";
  tripRows.forEach(t => {
    const parts = [t.from, t.to, t.shop1, t.shop2, t.shop3].filter(v => v && v.trim() && v !== "-" && v !== "จังหวัด" && v !== "ชื่อร้าน" && v !== "");
  ...
  const hint = document.getElementById("importHint"); if (hint) hint.style.display = "flex";
  dismissBanner();
  alert(`✅ นำข้อมูล ${tripRows.length} แถวจากแผนมาแล้ว`);
}

async function loadPlanById() {
  const id = document.getElementById("refPlanId")?.value?.trim(); if (!id) { alert("กรุณากรอก Plan ID"); return; }
  try {
...
async function loadActualDraft() {
  try {
    const { data: { session } } = await supabaseClient.auth.getSession(); if (!session) return;
    const { data, error } = await supabaseClient.from("actuals").select("id, ref_plan_id, start_date, end_date, rows, grand_total, status, allowance_rate, allowance_days, hotel_rate, hotel_nights, other_cost").eq("user_id", session.user.id).eq("status", "draft").order("updated_at", { ascending: false }).limit(1);
    if (error || !data || data.length === 0) return;
    const draft = data[0]; if (!Array.isArray(draft.rows) || draft.rows.length === 0) return;
    const fmtD = d => d ? new Date(d).toLocaleDateString("th-TH", { day: "numeric", month: "short", year: "numeric" }) : "-";
    ...
    document.getElementById("tableBody").innerHTML = "";
    draft.rows.forEach(r => { addRow(r.date || "", r.route || ""); const last = document.getElementById("tableBody").lastElementChild; const inp = last.querySelectorAll("input"); if (inp[2]) inp[2].value = r.note || ""; });
    calcTotal();
...
function addRow(date, route) {
  ...
  const tr = document.createElement("tr"), safeRoute = (route || "").replace(/"/g, "&quot;");
...
function calcTotal() {
  let total = 0; const rows = document.querySelectorAll("#tableBody tr");
...
function collectFormData() {
  const emp = document.getElementById("empName")?.value?.trim() || "", zone = document.getElementById("empZone")?.value?.trim() || "";
  if (!emp) { alert("ไม่พบข้อมูลพนักงาน"); return null; }
  const rows = []; let firstDate = "", lastDate = "";
  document.querySelectorAll("#tableBody tr").forEach((tr, i) => { const inp = tr.querySelectorAll("input"); const date = inp[0]?.value || ""; if (i === 0) firstDate = date; lastDate = date; rows.push({ date, route: inp[1]?.value || "", note: inp[2]?.value || "" }); });
  if (rows.length === 0) { alert("กรุณาเพิ่มข้อมูลอย่างน้อย 1 แถว"); return null; }
  const ar = parseFloat(document.getElementById("allowanceRate")?.value) || 0, ad = parseFloat(document.getElementById("allowanceDays")?.value) || 0;
  const hr = parseFloat(document.getElementById("hotelRate")?.value) || 0, hn = parseFloat(document.getElementById("hotelNights")?.value) || 0;
...
function loadDraftListLocal() {
  ...
  c.innerHTML = ""; drafts.forEach(d => { const el = document.createElement("div"); el.className = "draft-item"; const dateText = d.start ? new Date(d.start).toLocaleDateString("th-TH") : "-";
    el.innerHTML = `<div><div>📅 ${dateText}</div><div style="font-size:12px;color:#666">${d.rows?.length || 0} แถว</div></div><div style="display:flex;gap:6px"><button onclick="loadDraftLocal(${d.id})">แก้ไข</button><button onclick="deleteDraftLocal(${d.id})">ลบ</button></div>`; c.appendChild(el); });
}
function loadDraftLocal(id) {
  const drafts = JSON.parse(localStorage.getItem(STORAGE_KEY) || "[]"), d = drafts.find(x => x.id === id); if (!d) return;
  document.getElementById("tableBody").innerHTML = "";
  ...
  document.getElementById("allowanceRate").value = d.allowanceRate || ""; document.getElementById("allowanceDays").value = d.allowanceDays || "";
  document.getElementById("hotelRate").value = d.hotelRate || ""; document.getElementById("hotelNights").value = d.hotelNights || "";
  document.getElementById("otherCost").value = d.otherCost || ""; calculateSummary(); alert("✅ โหลด Draft แล้ว");
}
...
async function saveAndClose() {
  ...
  try {
    const { data: { session } } = await supabaseClient.auth.getSession(); if (!session) { alert("กรุณา Login ก่อน"); return; }
    ...
    const { error } = await supabaseClient.from("actuals").insert([payload]); if (error) throw error;
...
function calculateSummary() {
  ...
  const hr = parseFloat(document.getElementById("hotelRate")?.value) || 0, hn = parseFloat(document.getElementById("hotelNights")?.value) || 0;
  const oc = parseFloat(document.getElementById("otherCost")?.value) || 0;
  const el = document.getElementById("grandTotal"); if (el) el.value = (ar * ad + hr * hn + oc).toLocaleString("th-TH");
}
...
function openPreview() {
  ...
  let tRows = "";
  d.rows.forEach((r, i) => { const bg = i % 2 === 1 ? ' style="background:#f7f9fb"' : ""; tRows += `<tr${bg}><td>${fd(r.date)}</td><td style="text-align:left;padding-left:6px">${r.route || "-"}</td><td style="text-align:left;padding-left:4px">${r.note || ""}</td></tr>`; });
...
.dt th{background:#e8f5f4;color:#1a5550;padding:4px 4px;text-align:center;border:1px solid #b2d8d5;font-size:9.5px;font-weight:700}
.dt td{padding:3px 4px;border:1px solid #ccc;text-align:center;font-size:9.5px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
...
<div class="dw">
  ...
  <table class="ct">
    ...
    <tr class="tr"><th colspan="2">รวมเบิกทั้งหมด</th><th style="font-size:11px">${fm(d.grandTotal)} บาท</th></tr>
  </table>
  <div class="sg">
...
.sl{border-top:1px solid #555;padding-top:6px;margin-top:30px}
</style></head><body>
<div class="no-print">
...
console.log("✅ formActual.js v2.4 loaded");