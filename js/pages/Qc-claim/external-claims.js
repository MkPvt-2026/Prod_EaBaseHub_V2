// ============================================================
// external-claims.js  (v4.1 — FIXED sendToCEO upsert issue)
// หน้า QC ตรวจสอบเคลมลูกค้า (External Claims)
//
// ✅ Changelog v4.1:
//   1) FIX: sendToCEO ไม่พึ่ง unique constraint บน (source_table, source_id)
//      → เปลี่ยนเป็น check-then-insert/update แทน upsert
//   2) FIX: ถ้า claims update สำเร็จ แต่ approval_requests fail
//      → rollback claims qc_status กลับเป็น draft (ไม่ให้ค้างที่ waiting_ceo เปล่า ๆ)
//   3) IMPROVE: error message ชัดเจนขึ้น แสดงทั้ง 2 step
// ============================================================
...
function getQcFormData() {
  ...
  const productSource = document.getElementById("qcProductSource")?.value || "in_house";
  const rootCause = document.getElementById("qcClaimRootCause")?.value || "logistics";
...
function fillQcFormData(claim) {
  ...
  setInputValue("responsiblePerson", res.responsible_person ?? res.responsibility ?? "");
...
function setQcModalReadonly(isLocked) {
  ...
  document.querySelectorAll(selector).forEach((el) => { el.disabled = isLocked; });
...
// ============================================================
const qcSignature = {
  canvas: null, ctx: null, drawing: false, hasInk: false, lastPoint: null,
...
function updateSummaryCards() {
  ...
  const inProgress = allClaims.filter((c) => ["checking", "in_progress", "draft"].includes(normalizeStatus(c))).length;
  const waiting = allClaims.filter((c) => normalizeStatus(c) === "waiting_ceo").length;
  const approved = allClaims.filter((c) => ["approved", "exec_approved"].includes(normalizeStatus(c))).length;
  const rejected = allClaims.filter((c) => ["rejected", "exec_rejected"].includes(normalizeStatus(c))).length;
...
function populateCustomerFilter() {
  ...
  const customers = [...new Set(allClaims.map((c) => c.customer).filter(Boolean))]
  ...
  Object.entries(QTY_CHECKBOX_MAP).forEach(([qtyId, checkId]) => {
    ...
    if (checkEl && qtyEl) {
      checkEl.addEventListener("change", () => {
        ...
        if (checkEl.checked && Number(qtyEl.value || 0) === 0) qtyEl.value = 1;
...
function openApprovalDocFor(claimId) {
  ...
  if (!claim) { showToast("ไม่พบข้อมูลเคลม", "danger"); return; }
...
function buildApprovalDocCellHtml(claim) {
  if (!hasApprovalDocument(claim)) return `<span class="cell-no-media">—</span>`;
...
async function saveQcDraft() {
  ...
  try {
    ...
    await showSuccessPopup({
      ...
      message: "ระบบได้บันทึกข้อมูล QC เรียบร้อยแล้ว\nคุณสามารถกลับมาแก้ไขต่อภายหลังได้",
      buttonText: "ตกลง",
    });
    await loadClaims();
  } catch (err) {
...
function showSuccessPopup({
  title = "สำเร็จ", message = "ดำเนินการเรียบร้อยแล้ว",
  buttonText = "ตกลง", icon = "check_circle", autoCloseMs = 0,
} = {}) {
...
function showConfirmDialog({
  title = "ยืนยันการดำเนินการ", message = "ต้องการดำเนินการต่อหรือไม่?",
  noteLabel = "หมายเหตุ / ความเห็นเพิ่มเติม",
  notePlaceholder = "พิมพ์ข้อความ... (ไม่บังคับ)", initialNote = "",
  confirmText = "ยืนยัน", cancelText = "ยกเลิก",
  icon = "forward_to_inbox", variant = "primary", showNote = true,
} = {}) {
  return new Promise((resolve) => {
    ...
    function cleanup(result) {
      wrap.classList.add("closing");
      document.removeEventListener("keydown", onKey);
      setTimeout(() => { wrap.remove(); resolve(result); }, 180);
    }
    function onConfirm() { cleanup({ confirmed: true, note: (noteEl?.value || "").trim() }); }
    function onCancel() { cleanup({ confirmed: false, note: "" }); }
    function onKey(e) {
      if (e.key === "Escape") { e.preventDefault(); onCancel(); }
      else if (e.key === "Enter" && (e.ctrlKey || e.metaKey)) { e.preventDefault(); onConfirm(); }
    }
...
// ============================================================
// ✅ FIXED v4.1: sendToCEO — check-then-insert/update แทน upsert
// (ไม่ต้องพึ่ง unique constraint บน source_table+source_id)
// ============================================================
async function sendToCEO() {
  ...
  // เก็บสถานะเดิมไว้ rollback ถ้าจำเป็น
  ...
  try {
    ...
    // ─── STEP 2: เช็คว่ามี approval_requests สำหรับเคลมนี้แล้วหรือยัง ───
    ...
    if (lookupError) {
      console.warn("[sendToCEO] lookup existing approval_requests error:", lookupError);
      // ไม่ต้อง throw — ลอง insert ใหม่ต่อ
    }
    ...
    if (existing?.id) {
      ...
      // ยังไม่มี → INSERT (ใส่ created_at เพิ่ม)
      const { error } = await supabaseClient
        .from("approval_requests")
    ...
    if (approvalError) {
      // STEP 2 fail → rollback STEP 1 ไม่ให้ claims ค้างที่ waiting_ceo เปล่า ๆ
      console.error("[sendToCEO] approval_requests write failed, rolling back claims:", approvalError);
      ...
      throw new Error(
        ...
        `ระบบได้ rollback สถานะเคลมกลับเป็น "${prevQcStatus || "draft"}" แล้ว`
      );
    }
...
// ---------- Export ----------
const PRODUCT_SOURCE_LABELS = { in_house: "ผลิตเอง", trading: "ซื้อมาขายไป" };
const ROOT_CAUSE_LABELS = {
  logistics: "ขนส่ง", production: "ผลิต/โรงงาน", wrong_delivery: "ส่งผิดสเปก",
};

function buildExportRows(claims) {
  const rows = [[
    "เลขเคลม", "วันที่แจ้ง", "ผู้แจ้ง", "เขต/แผนก", "ลูกค้า", "สินค้า", "จำนวน",
    "ประเภทปัญหา", "รายละเอียด", "สถานะ QC", "ผล CEO", "วันที่รับเรื่อง",
    "หมายเหตุ QC", "ประเภทสินค้า", "ต้นเหตุ",
    "Grade A (คืนสต็อก)", "Grade B (Outlet)", "Grade R (บด/หลอม)", "Grade C (ทิ้ง)",
    "รวม", "สาเหตุหลัก", "ผู้รับผิดชอบ",
  ]];

  claims.forEach((c) => {
    const r = c.qc_result || {};
    const total = Number(r.grade_a_qty || 0) + Number(r.grade_b_qty || 0)
      + Number(r.grade_r_qty || r.repair_qty || 0)
      + Number(r.grade_c_qty || r.scrap_qty || 0);

    rows.push([
      getClaimNo(c), formatDate(c.claim_date), c.emp_name || "",
      c.area || "", c.customer || "", c.product || "", c.qty || "",
      normalizeClaimTypes(c.claim_types).join(", "), c.detail || "",
      getStatusLabel(c), c.exec_status || "", formatDateTime(c.picked_at),
      c.qc_comment || r.comment || "",
      PRODUCT_SOURCE_LABELS[r.product_source] || "",
      ROOT_CAUSE_LABELS[r.claim_root_cause] || "",
      r.grade_a_qty ?? 0, r.grade_b_qty ?? 0,
      r.grade_r_qty ?? r.repair_qty ?? 0, r.grade_c_qty ?? r.scrap_qty ?? 0,
      total, r.main_cause ?? r.defect_reason ?? "",
...
function exportExcel() { exportExcelAll(); }

function exportClaimCSV() {
  if (!currentClaim) { showToast("ยังไม่ได้เลือกเคลม", "warning"); return; }
  const rows = buildExportRows([currentClaim]);
  exportRowsAsCsv(rows, `${CLAIM_SCOPE}_${getClaimNo(currentClaim)}.csv`);
}

function exportCSV() { exportClaimCSV(); }
...
// ---------- Toast (fallback ถ้า HTML ไม่มี ea-toast-container) ----------
function showToast(message, type = "success", opts = {}) {
  // ถ้ามี global showToast (จาก HTML helper) ใช้ตัวนั้น
...
console.log(`✅ ${CLAIM_SCOPE}-claims.js (v4.1) loaded`);