...
// external-claims.js  (อัปเดต A/B/R/C + product_source + root_cause)
// หน้า QC ตรวจสอบเคลมลูกค้า (External Claims)
// ============================================================
...
// ============================================================
// QC Form Helpers  (Grade A / B / R / C  +  source + root cause)
// ============================================================

// Map: qty input id  →  checkbox id (สำหรับ sync ค่าเช็คบ็อกซ์)
...
// Trading = สินค้าซื้อมาขายไป → ไม่สามารถส่งบด/หลอม (Grade R) ได้
function applyProductSourceConstraint() {
  ...
  if (recycleRow) {
    ...
    recycleRow.title = isTrading
      ? "สินค้าซื้อมาขายไป — รีไซเคิลไม่ได้"
      : "";
  }

  // ถ้าเลือก trading แล้ว ต้องล้างค่า Grade R ทิ้ง
  if (isTrading) {
...
function fillQcFormData(claim) {
  ...
  // Grade quantities — รองรับทั้ง field ใหม่ (grade_r_qty) และเก่า (repair_qty/scrap_qty) เผื่อข้อมูลเก่า
  setInputValue("qcGradeAQty", res.grade_a_qty ?? 0);
  setInputValue("qcGradeBQty", res.grade_b_qty ?? 0);
...
function setupQcInputListeners() {
  ...
  // เมื่อเปลี่ยน product_source → apply constraint (Grade R)
  const sourceEl = document.getElementById("qcProductSource");
  if (sourceEl) {
...
// ---------- เปิดเอกสารอนุมัติ (delegate ไป approval-document.js) ----------
...
// Export PDF เคลมปัจจุบัน — delegate ไป ApprovalDocument.download (ถ้าพร้อม)
function exportPDF() {
  if (!currentClaim) {
    showToast("ยังไม่ได้เลือกเคลม", "warning");
    return;
